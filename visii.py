...
class StableDiffusionVisii(DiffusionPipeline):
    ...
    def __call__(
        ...
        # 6. Prepare latent variables,准备随机噪声，生成过程对一个随机噪声逐步去噪
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
        shape = (batch_size, num_channels_latents, height // self.vae_scale_factor, width // self.vae_scale_factor) #[bs,4,64,64]
    ...
    def text_projection2(self, cond_images, target_images, device='cuda'):
        with torch.no_grad():
            clip_model = CLIPModel.from_pretrained("/data/ml/clip-vit-large-patch14")
            processor = AutoProcessor.from_pretrained("/data/ml/clip-vit-large-patch14")
            clip_model.eval().to(device)
    ...
    def train(
        ...
        **kwargs,
        ...
        #prompt_embeds的shape是:torch.Size([3, 77, 768])
        ...
        for i in range(len(cond_list)):
            ...
            # cond_image_latents的shape是:torch.Size([3, 4, 64, 64])
            # target_image_latents的shape是:torch.Size([1, 4, 64, 64])
        ...
        # latents的shape是:torch.Size([1, 4, 64, 64])
        global_step = 0
        ...
        for i in range(0, 1000):
            noises.append(torch.randn(target_image_latents.shape).to(target_image_latents.device)) #torch.Size([1, 4, 64, 64])
        torch.save(torch.cat(noises), os.path.join(log_dir, exp_name, 'noises.pt')) #随机初始化噪声
        ...
        random.shuffle(total_index) #生成一个索引列表，并且随机打乱
        print(f'Train with {num_img} pair(s)')

        for index in tqdm(total_index): #1000步
            with accelerator.accumulate(prompt_embeds):
                ...
                noisy_latents = self.scheduler.add_noise(target_image_latents, noise, timesteps) #将噪声添加到目标图像的潜在表示中，得到Z_y_t
                # noisy_latents的shape是:torch.Size([1, 4, 64, 64])
                
                latent_model_input = torch.cat([noisy_latents] * 3) if do_classifier_free_guidance else noisy_latents
                # latent_model_input的shape是:torch.Size([3, 4, 64, 64])

                scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, timesteps)
                scaled_latent_model_input = torch.cat([scaled_latent_model_input, cond_image_latents], dim=1) #Z_y_t和Z_x
                # scaled_latent_model_input的shape是:torch.Size([3, 8, 64, 64])
                # prompt_embeds的shape是:torch.Size([3, 77, 768])
    ...
    def test(
        ...
        image = preprocess(image)
        height, width = image.shape[-2:]
        # image的shape是:torch.Size([1, 3, 512, 512])
        ...
        # num_images_per_prompt = 2 image_latents的shape是:torch.Size([6, 4, 64, 64]) 
        # image_latents的shape是:torch.Size([3, 4, 64, 64]) 
        ...
        # num_images_per_prompt = 2 latents的shape是:torch.Size([2, 4, 64, 64])
        # latents的shape是:torch.Size([1, 4, 64, 64])
        ...
        prompt_embeds = prompt_embeds.repeat(1, num_images_per_prompt, 1)
        prompt_embeds = prompt_embeds.view(bs_embed * num_images_per_prompt, seq_len, -1)
        # prompt_embeds的shape是:torch.Size([6, 77, 768])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # is applied for both the text and the input image.
                latent_model_input = torch.cat([latents] * 3) if do_classifier_free_guidance else latents
                #if latents[1,4,64,64],then latent_model_input[3,4,64,64]
    ...
    def get_specific_noise(
        ...
        for i, t in enumerate(timesteps):
            latent_model_input = torch.cat([latents] * 3) if do_classifier_free_guidance else latents
            # latent_model_input[3,4,64,64]

            # concat latents, image_latents in the channel dimension
            scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
            scaled_latent_model_input = torch.cat([scaled_latent_model_input, image_latents], dim=1)
            # scaled_latent_model_input [3,8,64,64]
            # prompt_embeds的shape是:torch.Size([3, 77, 768])
