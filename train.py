...
if __name__ == "__main__":
    ...
    print('Train with subfolders: ', subfolders) #Train with subfolders:  ['painting1']
    ...
    for folder in subfolders:
        ...
        cond_image = Image.open(before_path).resize((512, 512)).convert('RGB') #0_0.png
        target_image = Image.open(before_path.replace('_0.', '_1.')).resize((512, 512)).convert('RGB') #0_1.png

        exp_name = '{}_{}_{}'.format(config['exp']['init_expname'], folder, before_path.split('/')[-1]) 
        log_dir = os.path.join(args.log_dir, exp_name)#'logs/ip2p_painting1_0_0.png'
        ...
        pipe = StableDiffusionVisii.from_pretrained(model_id,torch_dtype=torch.float32).to("cuda") #instruct-pix2pix
        pipe.train(
            prompt=prompt,
            prompt_embeds=None,
        ...
        if config['exp']['eval']:
            checkpoints = [os.path.join(log_dir, 'prompt_embeds_{}.pt'.format(x)) for x in np.arange(0, config['hyperparams']['optimization_steps'], config['hyperparams']['eval_step'])] #(0,1000,100)
            after_images = np.concatenate([np.array(cond_image), np.array(target_image)], axis=1)
            location = os.path.join(log_dir, 'eval_{}'.format(config['hyperparams']['eval_step']) + '.png') #/logs/ip2p_painting1_0_0.png/eval_100.png
