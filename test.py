...
if __name__ == "__main__":
    args = argparser()
    model_id = "/data/ml/instruct-pix2pix"
    pipe = StableDiffusionVisii.from_pretrained(model_id, torch_dtype=torch.float32).to("cuda")
    pipe.scheduler = EulerAncestralDiscreteScheduler.from_config(pipe.scheduler.config)
    image_folder = args.image_folder #'./images'

    list_images = glob.glob(os.path.join(image_folder, '*_0.png')) +glob.glob(os.path.join(image_folder, '*_0.jpg'))
    #找到0_0.png和1_0.png 初始图片
    log_dir = os.path.join(args.log_path, args.log_folder) #'./logs/ip2p_painting1_0_0.png'
    os.makedirs('results', exist_ok=True)

    for img_path in list_images[1:2]: #list_images中的第二个元素，test图片
        ...
        #results/ip2p_painting1_0_0_1_0.png.png
        checkpoint = os.path.join(log_dir, 'prompt_embeds_{}.pt'.format(args.checkpoint_number))#用best模型
        opt_embs = torch.load(checkpoint) #torch.Size([3, 77, 768])

        after_images = []
        for i in range(0, args.number_of_row): #i = 0,1
            if args.hybrid_ins:
            ...
            for index in range(0, len(after_image)): #index = 0,1
